...
## 역할
feature 브랜치에서 버전을 업데이트하고, develop에 머지한 뒤 태그를 생성한다.
...
## Step 2: VERSION 파일 수정 + 커밋 (feature 브랜치에서)
...
```bash
git add VERSION
git commit -m "chore: bump version to v{새 버전}"
```

---

## Step 3: develop으로 이동 + 머지

```bash
git checkout develop
git merge {feature 브랜치명} --no-ff -m "merge: {feature 브랜치명}"
```
...
```
```
↩️ 머지를 중단했습니다. feature 브랜치로 돌아왔습니다.
```
...
| `git commit` (버전) | 1 | pre-commit hook 실패 → 수정 후 재시도 | 사용자에게 보고 |
| `git merge` (충돌) | 0 | 재시도 없음 — 사용자 수동 해결 필요 | 충돌 파일 목록 제공 |
| `git merge` (기타) | 1 | 충돌 외 오류 → 재시도 | 머지 abort 후 사용자에게 보고 |
